...
export default function LiveTradingPanel({active, symbol, analysis, masterTrade, sharedStatus, sharedConnections, onRefreshStatus}: Props) {
          ...
          : !readinessReady
            ...
            : status.consent?.active !== true
              ...
              : !status.armed
                ...
                : 'No LIVE blocker.'
  const liveState = status === null ? 'UNKNOWN' : emergency || recoveryRequired ? 'BLOCKED' : status.live_auto_trade ? 'RUNNING' : status.armed ? 'ARMED' : readinessReady ? 'READY' : 'LOCKED'
  ...
  if (masterTrade) return <section id="master-trade-live-terminal" className={`masterTradeLiveUx ${advancedOpen ? 'advanced-open' : 'operational'}`} aria-label="LIVE Auto Trade workspace">
    ...
    <div className="masterTradeLiveFlow" aria-label="LIVE Auto Trade progression"><span className={readinessReady ? 'complete' : 'current'}>1 <b>{readinessReady ? 'READY' : 'LOCKED'}</b></span><i /><span className={!status?.armed && readinessReady ? 'current' : status?.armed ? 'complete' : ''}>2 <b>ARM REQUIRED</b></span><i /><span className={status?.armed && !status?.live_auto_trade ? 'current' : status?.live_auto_trade ? 'complete' : ''}>3 <b>ARMED</b></span><i /><span className={status?.live_auto_trade ? 'current running' : ''}>4 <b>RUNNING</b></span></div>
