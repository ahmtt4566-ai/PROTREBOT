...
export default function LiveTradingPanel({active, symbol, analysis}: Props) {
  ...
  if (masterTrade) return <section className="masterTradeLiveUx" aria-label="LIVE Auto Trade workspace">
    ...
    <section className="masterTradeLiveConnection"><div><span className="masterTradeLiveKicker">LIVE CONNECTION</span><strong><i className={connected ? 'connected' : ''} /> {connected ? 'Connected' : 'Not connected'}</strong><small>{configured ? `Verified identity ${text(liveConnection?.fingerprint || status?.credentials?.fingerprint)}` : 'API settings are kept behind an expandable control.'}</small></div><button type="button" className="masterTradeLiveTextButton" onClick={() => setApiSettingsOpen(value => !value)}>{apiSettingsOpen ? 'CLOSE API SETTINGS' : 'API SETTINGS'}</button></section>
    {apiSettingsOpen && <section className="masterTradeLiveApiDrawer"><div className="masterTradeLiveApiHeader"><div><span className="masterTradeLiveKicker">SECURE CONNECTION</span><h3>LIVE API settings</h3></div><span>Secrets never return to the browser</span></div><div className="masterTradeLiveApiGrid"><label>API KEY<input type="password" autoComplete="new-password" value={credentials.apiKey} onChange={event => setCredentials({...credentials, apiKey: event.target.value})} /></label><label>API SECRET<input type="password" autoComplete="new-password" value={credentials.secretKey} onChange={event => setCredentials({...credentials, secretKey: event.target.value})} /></label></div><label className="masterTradeLiveCheck"><input type="checkbox" checked={credentials.accepted} onChange={event => setCredentials({...credentials, accepted: event.target.checked})}/><span>Store the secret only in the encrypted server vault.</span></label><div className="masterTradeLiveApiMeta"><span>Environment <b>Binance Futures Mainnet</b></span><span>Permissions <b>Futures only · withdrawals unsupported</b></span><span>Last verification <b>{date(status?.connection?.last_checked)}</b></span><span>Identity <b>{text(liveConnection?.fingerprint || status?.credentials?.fingerprint)}</b></span></div><div className="masterTradeLiveApiActions"><button type="button" onClick={testConnection} disabled={Boolean(busy) || !connections?.vault?.ready}>VERIFY CONNECTION</button><button type="button" onClick={saveCredentials} disabled={Boolean(busy) || !connections?.vault?.ready}>SAVE SECURELY</button><button type="button" onClick={connectReadOnly} disabled={Boolean(busy) || !configured}>CONNECT READ-ONLY</button></div></section>}

    <footer className="masterTradeLiveEmergency"><div><span className="masterTradeLiveKicker">EMERGENCY</span><strong><i className={emergency ? 'active' : ''} /> {emergency ? 'ACTIVE' : 'CLEAR'}</strong><small>{emergency ? (status?.emergency?.reason || 'LIVE execution is blocked.') : 'Backend emergency control is inactive.'}</small></div><button type="button" className="dangerAction" onClick={emergencyStop} disabled={Boolean(busy)}>EMERGENCY STOP</button></footer>
    <div className={`liveNotice ${notice.kind}`}><ShieldCheck/><span>{notice.text}</span><button type="button" onClick={() => void refresh(false)} disabled={Boolean(busy)}><RefreshCw/> REFRESH</button></div>
  ...
  return <section className="liveTradingPanel liveTerminal liveUx" aria-label="LIVE Trading Operations Terminal">
    ...
    <section className="liveUxCard liveUxAuto"><header><Power/><div><small>AUTO TRADE / LIVE ARM</small><h3>Supervised live automation</h3></div><b>{status?.live_auto_trade ? 'ON' : 'OFF'}</b></header><div className="liveUxActionRow"><button type="button" onClick={autoToggle} disabled={Boolean(busy) || !autoReady || Boolean(status?.live_auto_trade)}><Power/> ENABLE AUTO TRADE</button><button type="button" onClick={stopAutoTrade} disabled={Boolean(busy) || !status?.live_auto_trade}><Power/> STOP AUTO TRADE</button></div><p className="liveUxReason"><strong>Reason:</strong> {status?.live_auto_trade ? 'Automatic trading is enabled by backend state.' : status?.auto?.last_decision || blocker}</p></section>
